...
const Product = () => {
    ...
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        ...
        if (response.data.status === 1) {
          ...
          showToast(`Error deleting product: ${response.data.message}`, false);
        }
      } catch (error) {
        showToast('An unexpected error occurred while deleting product.', false);
      }
    }
  };
  ...
  {editingProductId === product.productID ? (
    ...
    categoriesList.find((category) => category.categoryId === product.categoryId)?.categoryName || ''
                  ...
                  <td className="border border-gray-800 p-2" style={{ width: '250px' }}>
                    {editingProductId === product.productID ? (
                      <>
                        <button
